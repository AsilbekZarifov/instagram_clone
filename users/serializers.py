...
from .models import User, UserConfirmation, VIA_EMAIL, VIA_PHONE, NEW, CODE_VERIFIED, DONE, PHONE_EXPIRE, PHOTO_DONE
...
class ResertPassordSerializer(serializers.ModelSerializer):
    id = serializers.UUIDField(read_only=True)
    ...
    def validate(self, data):
        ...
        if password != confirm_password:
            raise ValidationError(
                {
                    "success":False,
                    "message":"Parollaringiz qiymatlari bir-biriga teng emas "
                }
            )
        if password:
            validate_password(password)
    ...
    def update(self, instance, validated_data):
        ...
        return super(ResertPassordSerializer, self).update(instance, validated_data )